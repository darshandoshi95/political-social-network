...
def get_users(twitter, screen_names):
    ...
    ###TODO
    users=[]
    requests= robust_request(twitter,'users/lookup',{'screen_name':screen_names})
    users=[r for r in requests]
    return users
    pass
...
def friend_overlap(users):
    ...
    for i in range(k):
        for j in range(i+1,k):
            count=0
            for friend1 in users[i]['friends']:
                for friend2 in users[j]['friends']:
                    if friend1 == friend2:
                        count+=1
            tuples=(users[i]['screen_name'],users[j]['screen_name'],count)
            friend_overlap.append(tuples)
    ...
    for u in users:
        if(u['screen_name']=='HillaryClinton'):
               userH=u['friends']
                ...
                userD=u['friends']
    user= list(set(userH).intersection(userD))
    if (len(user)>0):
          follow_id=user[0]
...
def create_graph(users, friend_counts):
    ...
    graph = nx.Graph()
    edges = []
    #sorted_count = sorted(friend_counts.items(), key=lambda x: x[1], reverse=True)
    for u in users:
        for i in u['friends']:
...
def draw_network(graph, users, filename):
    ...
    graph_labels = {n : n if n in screen_name else '' for n in graph.nodes()}
    plt.figure(figsize=(15,15))
    nx.draw_networkx(graph,labels=graph_labels,alpha=.9,width=.5 ,font_size =19, font_color = 'b',node_size=100)
